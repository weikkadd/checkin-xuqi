        ...
        try:
            ...
            if "accounts.google.com" in current_url:
                allow_btn = await page.query_selector(
                    'button:has-text("Allow"), button:has-text("允许"), button:has-text("Continue")'
                )
                if allow_btn:
            ...
            for sel in renew_selectors:
                ...
                if btn:
                    ...
                    if visible and not disabled:
                        ...
                        renew_clicked = True
                        break
                    elif visible and disabled:
                        raise Exception(f"续期按钮已禁用，可能未到续期窗口（到期前2天开放）")

            if not renew_clicked:
                raise Exception("续期按钮未出现（可能未到续期时间窗口）")
            ...
            success = await page.evaluate("""
                () => {
                    ...
                    return /续期成功|Renewal successful|已续期|操作成功/i.test(txt);
                }
            """)
